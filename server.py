...
def find_afterparties():
    ...
    distance = request.args.get('distance') + request.args.get('measurement')
    ...
    # the form data.
    # - (Make sure to save the JSON data from the response to the data variable
    # so that it can display on the page as well.)

    payload = {'token': os.environ['EVENTBRITE_TOKEN'], 
                'q': query, 'location.address': location,
...
def create_eventbrite_event():
    ...
    payload = {'event.name.html': name, 'event.start.utc': start_time,
                ...
                'event.end.timezone': timezone, 'event.currency': currency}
